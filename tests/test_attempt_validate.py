...
def _write_attempt(root, **overrides):
    payload = {
    ...
    _write_json(root / "attempts" / f"{payload['id']}.json", payload)


def test_valid_attempt_record_passes_validation(tmp_path):
...
def test_attempt_links_must_reference_known_records(tmp_path):
    _write_base_workspace(tmp_path)
    _write_attempt(
        tmp_path,
    ...
    assert any("unknown harness_id: missing-harness" in message for message in messages)
...
def test_attempt_must_include_source_ids(tmp_path):
    _write_base_workspace(tmp_path)
    _write_attempt(tmp_path, source_ids=[])

    messages = validate_workspace(tmp_path)

    assert any("attempt must include source_ids" in message for message in messages)
