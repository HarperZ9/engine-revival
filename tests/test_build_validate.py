...
def _write_build(root, **overrides):
    payload = {
    ...
    _write_json(root / "builds" / f"{payload['id']}.json", payload)


def test_valid_build_record_passes_validation(tmp_path):
...
def test_build_links_must_reference_known_records(tmp_path):
    _write_base_workspace(tmp_path)
    _write_build(
        tmp_path,
...
def test_build_must_include_source_ids(tmp_path):
    _write_base_workspace(tmp_path)
    _write_build(tmp_path, source_ids=[])
