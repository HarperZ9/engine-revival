...
Bytes, not decoded text. A checkout that rewrote the line endings reads back
identical through Python's universal newlines and would pass a text comparison,
so docs/art is pinned to LF in .gitattributes and the file is read as it sits on
disk.
...
ART = ROOT / "docs" / "art"
SCHEMA = "engine-revival.repo-art/v1"
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "engine-revival", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('engine-revival') % 100000:05d}"
